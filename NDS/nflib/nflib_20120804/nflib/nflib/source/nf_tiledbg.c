...
void NF_LoadTiledBg(const char* file, const char* name, u16 width, u16 height) {
	...
	u16 bx = width - ((width >> 8) << 8);
	u16 by = height - ((height >> 8) << 8);
	if ((bx > 0) || (by > 0)) {
		NF_Error(115, file, 0);
	}
...
u16 NF_GetTileOfMap(u8 screen, u8 layer, u16 tile_x, u16 tile_y) {
	...
	// Obten los bytes
	u16 lobyte = *(NF_BUFFER_BGMAP[NF_TILEDBG_LAYERS[screen][layer].bgslot] + address);
	u16 hibyte = *(NF_BUFFER_BGMAP[NF_TILEDBG_LAYERS[screen][layer].bgslot] + (address + 1));
...
void NF_BgGetPalColor(u8 screen, u8 layer, u8 number, u8* r, u8* g, u8* b) {
	...
	// Obten los bytes
	u16 lobyte = *(NF_BUFFER_BGPAL[NF_TILEDBG_LAYERS[screen][layer].bgslot] + (number << 1));
	u16 hibyte = *(NF_BUFFER_BGPAL[NF_TILEDBG_LAYERS[screen][layer].bgslot] + ((number << 1) + 1));
