...
inline void ASM_loadSprite(ASMSys *system, const char *file, int sprite_num){
	char newLine[256];
	...
	if ( system->video.tempSpr  == NULL ){
     ...
     SDL_SetColorKey(system->video.sprite[sprite_num], SDL_SRCCOLORKEY, SDL_MapRGB(system->video.sprite[sprite_num], 255, 0, 255));
	}
}
...
void ASM_flip(ASMSys *system, int screen){
